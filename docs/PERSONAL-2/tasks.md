...
⚠ Task бүрийн `Гүнзгийрүүлэлт` мөр нь `plan.md §11…§14`-ийн гэрээг лавлана — тэндээс
ГАДУУР схем, талбар, токен, event ЗОХИОХГҮЙ.
⚠ Баримтын сүүлд: **Hefesto-гийн ID зураглал** (`513…549` ↔ `T-01…T-37`, хэмжээ,
параллель багц) · **Task тус бүрийн тестийн нэгжүүд** (нэрлэсэн жагсаалт) ·
**Шийдвэрийн дельта** (P-15…P-26 ямар task-д хүрсэн).
...
- **Гүнзгийрүүлэлт (`plan.md` P-22 · P-23 · P-24 · §11.2):** `CosmeticUnlockSource` нь `{ kind, refId, value? }` объект бөгөөд `kind` нь ЯГ 5 утгатай; `AchievementDefinition.predicate`-д заавал биш `ref: string`; `ReplayLogEntry.kind` нь 5 утгатай enum (энэ ажилд зөвхөн `boss` бичигдэнэ). Схем бүрийн бүрэн хэлбэрийг `plan.md §11.2` заана — энд ЗОХИОХГҮЙ.
...
- **AC:** SVX-1 (схем), QX-7
- **Гүнзгийрүүлэлт (`plan.md §11.1`):** `GameState`-ийн дельта нь ЯГ 10 өөрчлөлт. Тэдгээрт `dungeonStats` (P-15) ба `respecAt: string \| null` — **ГАНЦ** талбар, мод тутам БИШ (P-19) орно; `masteryPoints` нь **үлдэгдэл**, олдсон нийт нь гаргагдана (P-20). `bossAttempts[]`-д `difficulty` нэмэгдэнэ. Тогтмолд `REP_BASE = {main:3, boss:3, dungeon:2, side:2}` (P-16) орно. `unlockedCosmeticIds` · `personalBests` · `chainProgress` талбар НЭМЭХГҮЙ.
...
- **Гүнзгийрүүлэлт:** хуучин `bossAttempts[]` бичлэг бүрд `difficulty: 'standard'` бичнэ; `dungeonStats` нь дууссан dungeon id бүрд `{ lastPassedDate: null }` (P-15 — `null` = refresher-т нэр дэвшихгүй); `daysBetween(a, b)` туслахыг `result.ts`-д `dayOf`-ийн хажууд нэмнэ (P-18); `replayLog`-ийн ЦОРЫН ГАНЦ бичигч нь `boss.ts` (P-24).
...
- **Гүнзгийрүүлэлт (P-17 · `plan.md §13.2`):** mastery-д **ОЛГОГДСОН** XP очно (SQ-2-ийн бууралтын ДАРААХ утга), контентын суурь XP биш. Roll-up-ийн байрлал нь `claimQuest`-ийн `3a` алхам — **амжилтын үнэлгээнээс ӨМНӨ**, эс бөгөөс `masteryLevel` предикаттай амжилт нэг үйлдэл хоцорно.
...
- **AC:** RET-5, RET-6
- **Гүнзгийрүүлэлт (P-16 · `plan.md §12.3`):** `repAward(track, n) = floor(REP_BASE[track] × m^(n−1))`; side quest-д `2 · 1 · 0 · 0 …`. Rep-ийн байрлал нь `4a` алхам — амжилтын үнэлгээнээс ӨМНӨ. Тоон утга нь **H-5**-ийн хүний баталгаанд өгөгдсөн; өөрчилвөл энэ task-ийн ганц тогтмол солигдоно.
...
- **AC:** RET-2, RET-3
- **Гүнзгийрүүлэлт (P-21):** явцын ШИНЭ талбар БАЙХГҮЙ — дараалал нь `sideQuestStats[stepId].lastCompletedAt` буурахгүй эсэхээр шалгагдана. `chains.json`-д `world: 1..5` талбар байх ёстой (`RET-3`-ийн таазыг тооцоход).
...
- **Гүнзгийрүүлэлт (`plan.md §12.2`):** hard `mastery` босго 60 нь `BossScores`-ийн дээд нийлбэртэй тэнцэх тул **ТӨГС оноо** шаардана — мэдэгдэж буй үр дагавар, спекийн тоо өөрчлөгдөхгүй. Урвуулалтын тест нь 0..60 **бүх 61 утгаар** гүйнэ. `appendReplay`-ийн дуудлага нь бүх кодын бааз дээр ЭНЭ ганц газар (P-24).
...
- **Гүнзгийрүүлэлт (P-19 · P-20):** cooldown нь **глобал** (`respecAt` ганц талбар — **H-6**); `masteryPoints` нь үлдэгдэл бөгөөд `үлдэгдэл + зарцуулсан = олдсон нийт` инвариантыг тестээр хаана.
...
- **AC:** RET-4
- **Гүнзгийрүүлэлт (P-15 · P-18 · `plan.md §12.4`):** нэр дэвшилт нь `dungeonStats[id].lastPassedDate`-ээс тооцогдоно; `null` бол нэр дэвшихгүй. Хугацаа нь `daysBetween`-ээр. Оруулах БАЙРЛАЛ: `pickDailyMission`-ий `TIERS` гогцоо дууссаны ДАРАА, `return null`-ийн ӨМНӨ.
...
- **Гүнзгийрүүлэлт (P-25 · `plan.md §12.5`):** `campLayout = { slots: Record<CosmeticSlot, string \| null> }` — ЯГ 6 түлхүүр; танихгүй slot/id → `INVALID_INPUT`, нээгдээгүй cosmetic → `PREREQ_NOT_MET`. Нээлтийн үнэлгээний kind тутмын хүснэгт нь `plan.md §12.5`.
...
- **AC:** RET-7 (домэйн тал)
- **Гүнзгийрүүлэлт (P-23):** предикатад заавал биш `ref` нэмэгдэнэ (`masteryLevel` → tag, `guildRank` → guildId, `bossPersonalBest` → bossId; `ref` байхгүй = «дурын нэг»). Хувийн дээд амжилт нь хоёр difficulty-гийн **НЭГДСЭН** багц дээрх max.
...
- **AC:** OFF-7
- **Гүнзгийрүүлэлт (`plan.md §13.1`):** payload ба боломжит татгалзал бүрийн зураглал §13.1-д. `bossAttempt.difficulty` нь **заавал биш** — хуучин `actionQueue`-д хадгалагдсан үйлдэл серверт хожим хүрэхэд эвдрэхгүй.
...
- **AC:** COS-1, COS-2
- **Гүнзгийрүүлэлт (P-22):** `unlockSource` нь `{ kind, refId, value? }` объект, `kind` нь ЯГ 5 утгатай. Prestige цол ба streak шагналын cosmetic нь `kind: 'achievement'`-ээр `T-24`-ийн амжилтад заана — шинэ kind НЭМЭХГҮЙ.
...
- **AC:** RET-7
- **Гүнзгийрүүлэлт (P-22 · P-23):** `prestigeCount` ба `streakDays` предикаттай амжилт ЗААВАЛ байна — эс бөгөөс `T-23`-ийн prestige/streak cosmetic өнчирч `COS-2` УНАНА. Шинэ kind-ууд `ref`-ийг ашиглана.
...
- **AC:** QX-2, QX-4, OFF-7
- **Гүнзгийрүүлэлт:** серверийн гэрээний тест нь `plan.md §13.1`-ийн payload ба §13.2-ийн `data` хэлбэрийг шалгана — сервер тал өөрийн хувилбар тодорхойлохгүй.
...
- **Гүнзгийрүүлэлт (P-26 · `plan.md §14.1` · §14.2):** токен **9 → 19** — `styles.css`-ийн 14 хатуу hex нь `theme.ts`-ийн хүснэгтэд шилжинэ, эс бөгөөс контрастын тест тэдгээрийг ХАРАХГҮЙ. Шалгалтын хэмжээ: **19 хос × 6 палитр × 2 багц = 228**; ≥4.5 (14 хос) ба ≥3.0 (5 хос) ангилал нь §14.2-ийн ил жагсаалт.
...
- **Гүнзгийрүүлэлт (`plan.md §14.3`):** 12 мөрийн бүртгэл (event → класс → Hz → `aria-live` текстийн эх) нь §14.3-т бүрэн бичигдсэн. `sound.ts`-ийн `CUE_EVENTS` нь бүртгэлээс **ГАРГАГДАНА** — хоёр дахь жагсаалт үлдэхгүй.
...
## Hefesto-гийн ID зураглал

Ажлын нэгжүүд Hefesto-д аль хэдийн бүртгэгдсэн. **Дугаарлалт ТОГТМОЛ** — энэ баримтын
хожмын хувилбарууд task НЭМЭХГҮЙ, НЭГТГЭХГҮЙ: ID нь дараагийн шатуудын хаяг.

| Task | Hefesto ID | Давалгаа | Хэмжээ | Хамаарал |
|---|---|---|---|---|
| T-01 | 513 | W0 | M | — |
| T-02 | 514 | W0 | S | — |
| T-03 | 515 | W0 | M | — |
...
| T-08 | 520 | W2 | M | T-06 |
| T-09 | 521 | W2 | M | T-08 |
| T-10 | 522 | W2 | S | T-08, T-11 |
| T-11 | 523 | W2 | M | T-06 |
| T-12 | 524 | W2 | M | T-06 |
| T-13 | 525 | W2 | L | T-06 |
| T-14 | 526 | W2 | L | T-08, T-19, **H-1** |
| T-15 | 527 | W2 | S | T-06 |
| T-16 | 528 | W2 | M | T-08, T-11, T-13 |
| T-17 | 529 | W2 | M | T-08, T-11, T-12, T-13 |
| T-18 | 530 | W2 | M | T-08, T-14, T-16 |
| T-19 | 531 | W3 | M | T-05, **H-1** |
| T-20 | 532 | W3 | L | T-05 |
| T-21 | 533 | W3 | S | T-05, **H-2** |
| T-22 | 534 | W3 | M | T-05 |
| T-23 | 535 | W3 | L | T-05, T-19, T-20, T-21, T-24 |
| T-24 | 536 | W3 | L | T-17 |
| T-25 | 537 | W3 | M | T-03, T-19…T-24 |
| T-26 | 538 | W4 | M | T-18, T-25 |
| T-27 | 539 | W5 | L | T-05 |
| T-28 | 540 | W5 | L | T-18, T-27 |
| T-29 | 541 | W5 | M | T-09, T-11, T-27 |
| T-30 | 542 | W5 | M | T-16, T-23, T-27 |
| T-31 | 543 | W5 | L | T-14, T-19, T-27, **H-1** |
| T-32 | 544 | W5 | M | T-13, T-20, T-27 |
| T-33 | 545 | W5 | S | T-18, T-27, T-28 |
| T-34 | 546 | W5 | M | T-29…T-33 |
| T-35 | 547 | W6 | M | T-26, T-34 |
| T-36 | 548 | W7 | S | T-18, T-25 |
| T-37 | 549 | W7 | M | T-35, T-36 |

**Хэмжээний утга:** `S` = нэг суулт, ганц файл · `M` = хэд хэдэн файл эсвэл шинэ модуль
+ тест · `L` = шинэ дэд систем эсвэл ≥40 контентын элемент. Хэмжээ нь **хугацааны
амлалт БИШ** — параллель ажиллах боломжийг үнэлэх хэрэгсэл.

**Параллель хийж болох багцууд:** `{T-02, T-03, T-04}` (W0, хамааралгүй) ·
`{T-08, T-11, T-12, T-13, T-15}` (W2-ийн эхний давхарга, бүгд зөвхөн T-06-оос хамаарна) ·
`{T-19, T-20, T-21, T-22}` (W3 контент) · `{T-27, T-28}` дууссаны дараа
`{T-29, T-30, T-31, T-32, T-33}` · `T-26` нь W5-ийн бүх task-тай зэрэгцэнэ.

## Task тус бүрийн тестийн нэгжүүд

`plan.md §8`-ийн дуусгах дүрмийн 1 дэх заалт: тест **эхлээд унаж байсан** байх ёстой.
Доорх нь тэр тестүүдийн **нэрлэсэн жагсаалт** — хэрэгжүүлэгч нэр нэмж болно, ХАСАХГҮЙ.
Тоо нь `plan.md §15`-ийн төсөвтэй нийцнэ.

| Task | Тестийн нэгжүүд (нэр товчлол) | Төсөв |
|---|---|---|
| T-01 | `contracts.yaml` задарна · `version = 1.2.0` · enum утга бүр ЯГ нэг тодорхойлолттой | 3 |
| T-02 | суурь хэмжээ хэмжигдэнэ · одоогийн ≤ суурь × 1.3 · тогтмолыг 10% бууруулахад УНАНА | 3 |
| T-03 | CLI exit 0 · зөрчил тарихад exit ≠ 0 + зам хэвлэгдэнэ · CLI ба vitest НЭГ модуль дуудна | 4 |
| T-04 | гадаад origin тэг · `tutorialRefs` үл хамаарна · beacon/SDK байхгүй · `dependencies` хоосон · сканнер тус бүрд «өөрөө хазна» | 8 |
| T-05 | шинэ схем бүрийн эерэг/сөрөг · hard босго ТООЦОГДОНО (41/52/60) · тогтмол давхардаагүй · `architecture.test.ts` ногоон | 10 |
| T-06 | v1 → v2-д level/xp/quest/inventory/streak/skill ЯГ хэвээр · 10 шинэ талбар бүрийн анхдагч · `difficulty:'standard'` буцаан бичигдсэн · `dungeonStats` нь дууссан id бүрд · 501 бичлэгийн FIFO · `daysBetween` хилийн утга · SV-2/3/4 ногоон хэвээр | 18 |
| T-07 | round-trip нь шинэ талбар бүрийг хадгална · v1 export → import нь migration-аар · parity зөрөхөд УНАНА | 5 |
| T-08 | 7 track бүрийн босгын хүснэгт · XP нэвчихгүй · level 9-д prestige → `PREREQ_NOT_MET` + төлөв хэвээр · prestige-ийн дараа оноо буураагүй · `earnedMasteryPoints` томьёо | 12 |
| T-09 | 2 tag-тай quest → track тутамд бүтэн XP · давтан side quest-д ОЛГОГДСОН XP (P-17) · roll-up нь амжилтын үнэлгээнээс ӨМНӨ · PRG/SQ-ийн тест ногоон | 8 |
| T-10 | 5 модуль бүр mastery/rep/campLayout/replayLog уншихгүй · `progression.ts`-д `state.mastery` тарихад УНАНА | 6 |
| T-11 | tag → guild зураглал контентоос · `repAward` хүснэгт (2·1·0·0) · давтагдахгүй контент 2 дахь удаа rep өгөхгүй · rep хэзээ ч буураагүй · rep нь амжилтаас ӨМНӨ | 8 |
| T-12 | 1→2→4 дуусахгүй · 1→2→3→4 нь ЯГ нэг `CHAIN_COMPLETED` · урвуу дараалал (4→3→2→1) дуусахгүй · 4-р алхмыг давтахад bonus давхардахгүй · `bonusXp` тааз | 7 |
| T-13 | 40/41 · 51/52 · 59/60 хилүүд · 0..60 бүх утгын урвуулалтгүй байдал · дээд амжилт буурахгүй · 10 дараалсан rematch · replay бичлэг хоёр difficulty-д · BS-3-ийн формат хэвээр | 14 |
| T-14 | capstone-ийн 2³ хослол (мессеж дутсан нөхцөлийг НЭРЛЭНЭ) · tier-1 = skillPoint, tier-2/3 = mastery point · track таарахгүй → `PREREQ_NOT_MET` · 6 хоног → `RESPEC_ON_COOLDOWN` + төлөв ХЭВЭЭР · 7 хоног → зөвшөөрнө · respec-ийн буцаалт яг тэнцүү · `үлдэгдэл + зарцуулсан = олдсон` инвариант | 16 |
| T-15 | `lastPassedDate = null` → нэр дэвших**гүй** · 13 хоног → үгүй, 14 хоног → тийм · 1–3 эрэмбэд нэр дэвшигч байвал refresher хүрэхгүй · ижил оролт → ижил гаралт · DM-1…3 ногоон | 7 |
| T-16 | `campLayout` устгахад бусад утга хэвээр · 5 `kind` тутмын нээлт/үл нээлт · нээгдээгүй cosmetic зүүх → `PREREQ_NOT_MET` · streak 3/7/14/30-ийн шагнал · ACH-2 ногоон | 12 |
| T-17 | 5 шинэ `kind` тутмын хилийн тест · `ref` байхгүй = «дурын нэг» · `bossPersonalBest` нь хоёр difficulty-гийн max · дахин олгогдохгүй | 9 |
| T-18 | 3 action × (буруу payload → `INVALID_INPUT`, зөв payload → хүлээгдсэн event) · `updateSettings`-ийн 2 шинэ талбар · `soundVolume` мужаас гарахад татгалзана · дискпетчер дутуу бол typecheck УНАНА | 10 |
| T-19 | хуучин 24 id/title/description/cost ЯГ хэвээр · track тутамд ≥1/≥1/ЯГ 1 · нийт ≥28 · tier алгасахад УНАНА · мөчлөг тарихад УНАНА | 7 |
| T-20 | дэлхий тутамд `track:'boss'` ЯГ 1 · MQ-5-ийн талбар бүр · `boss-strange-room` 3-р дэлхийд хэвээр · шинэ `tutorialRefs` зохиогдоогүй | 6 |
| T-21 | 7 tag → 4 guild, давхцалгүй/дутуугүй · `placeholder` тэмдэглэгээ байгаа · `buildPack()`-д `guilds` орсон | 4 |
| T-22 | ≥3 chain × ЯГ 4 алхам · side quest хоёр chain-д орохгүй · давхцал тарихад УНАНА · `bonusXp` тааз хэтрэхэд УНАНА · `world` талбарын нийцэл | 6 |
| T-23 | ≥60 элемент · slot тутамд ≥5 · `effect = 'cosmetic'` · байхгүй id руу заасан `unlockSource` тарихад УНАНА · 5 kind бүр ашиглагдсан · `loot.json` хөндөгдөөгүй | 8 |
| T-24 | ≥40 амжилт · `prestigeCount` ба `streakDays` предикат БАЙНА (T-23-ийн зангуу) · предикат бүр бодит төлөвт үнэлэгдэнэ · давхардсан id тарихад УНАНА | 7 |
| T-25 | `[C]` дүрэм бүрд «зөрчил тарихад exit ≠ 0» · CLI ба vitest ижил дүгнэлт өгнө | 12 |
| T-26 | 3 action + 4 event нь серверийн гэрээнд · `/api/content/pack` < 2MB · хүчингүй контент → health 503 · сервер тал XP/tier дахин тооцоолоогүй | 10 |
| T-27 | 228 контрастын баталгаа (19 хос × 6 палитр × 2 багц) · токеныг бүдгэрүүлэхэд УНАНА · 2 палитрын DOM snapshot ЯЛГААГҮЙ · `colorBlindSafe` нь layout/текст хөндөөгүй · хатуу hex үлдсэн эсэхийг сканнердана | 12 |
| T-28 | 12 event бүрийн бүртгэл (нэгийг хасахад УНАНА) · `reducedMotion` үед класс нэмэгдээгүй · `soundVolume = 0` үед oscillator үүсээгүй · `fx.ts`-ээс гадна анимацийн класс нэмэх зам хориотой · 8 event зэрэг → ≤3 анимац, 8 текст мэдэгдэл · анимац бүр ≤300ms, зөвхөн transform/opacity | 14 |
| T-29 | 7 mastery bar + guild rank нэг дэлгэцэнд · UI-2-ийн элемент бүр хэвээр · `architecture.test.ts` ногоон | 5 |
| T-30 | ≥60 элемент жагсаагдсан · нээгдээгүй бүрд эх сурвалжийн текст · 9 дэх маршрут гарнаас хүрнэ | 5 |
| T-31 | capstone түгжээтэй үед дутсан нөхцөл НЭРЛЭГДСЭН · cooldown үед respec товч тайлбартай идэвхгүй · prestige товч зөвхөн level 10-д · node бүрийн валют ил | 8 |
| T-32 | hard сонгоход босго `41/52/60` харагдана · дээд амжилт буураагүй · rematch 3 удаа · difficulty нь өнгөөр БИШ текст/дүрсээр | 6 |
| T-33 | toggle → save → дахин ачаалахад хадгалагдсан · `soundVolume = 0` үед дуугүй · одоогийн settings тест ногоон | 5 |
| T-34 | шинэ визуал төлөв бүр текст/дүрстэй · `div`+`onclick` тарихад сканнер УНАНА · 3 өргөнд `scrollWidth ≤ clientWidth` · дэлгэц бүрт axe critical 0 | 10 |
| T-35 | сервергүй үед шинэ систем бүр ажиллана · бүх `fetch` reject үед uncaught 0 · офлайн төлөв ТЕКСТЭЭР · үйлдэл дараалалд хадгалагдана · PII шаардаагүй | 8 |
| T-36 | ×1 ба ×10 пакетын харьцаа шугаман · ×10 дээр claim ≤50ms | 3 |
| T-37 | тестийн тоо буураагүй · `skip` 0 · build warning-гүй · bundle ≤130% · архитектурын хил бүр | 6 |

⚠ Багана дахь **Төсөв** нь `it(...)` блокийн ойролцоо тоо — AC БИШ. Цорын ганц хатуу
дүрэм нь `QX-1`: нийт тоо БУУРАХГҮЙ.

## Шийдвэрийн дельта — `plan.md` P-15…P-26 ямар task-д хүрсэн

Гүнзгийрүүлэлтийн шатанд нэмэгдсэн 12 шийдвэр нь **шинэ task үүсгээгүй**, зөвхөн
одоо байгаа task-уудын хийх ажлыг тодотгосон:

| Шийдвэр | Хүрсэн task |
|---|---|
| P-15 (`dungeonStats.lastPassedDate`) | T-05, T-06, T-15 |
| P-16 (`REP_BASE` ба бууралт) | T-11 |
| P-17 (mastery-д олгогдсон XP) | T-09 |
| P-18 (`daysBetween`) | T-06, T-14, T-15 |
| P-19 (глобал `respecAt`) | T-05, T-06, T-14 |
| P-20 (`masteryPoints` = үлдэгдэл) | T-05, T-08, T-14 |
| P-21 (chain-ийн явц гаргагдана) | T-12 |
| P-22 (`unlockSource` объект, 5 kind) | T-01, T-05, T-23, T-24 |
| P-23 (предикатын `ref`) | T-01, T-05, T-17, T-24 |
| P-24 (replay бичигч ганц) | T-01, T-06, T-13 |
| P-25 (`campLayout.slots`) | T-05, T-16, T-30 |
| P-26 (токен 9 → 19) | T-27 |
